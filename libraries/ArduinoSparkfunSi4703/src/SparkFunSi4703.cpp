...
{ 
  ...
  while(completedCount < 4 && millis() < endTime) {
	  readRegisters();
    //Esta linea sirve para verificar si hay un grupo nuevo de RSD
    //Para ser leido
	  if(si4703_registers[STATUSRSSI] & (1<<RDSR)){
      ...
      delay(4); //Wait for the RDS bit to clear
	}
	else {
	  delay(3); //From AN230, using the polling method 40ms should be sufficient amount of time between checks
	}
  }
	if (millis() >= endTime) {
		buffer[0] ='\0';
...
void Si4703_Breakout::si4703_init() 
{
  ...
  si4703_registers[SYSCONFIG3] |= (1<<SPACE_SNR);//Set SNR threshold
